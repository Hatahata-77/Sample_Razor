...
(function ($) {
    ...
    $.jgrid.extend({
        ...
        //2018.10.12_ADD S レスポンスアップ対応で内部変数にアクセスできるようにしたが本来はよくないので消すかも
        , _getGrid: function () {
            var $t = this[0];
            return $t;
        }
		...
		return this.each(function(){
			...
			if (o.useColSpanStyle) {
				...
				$htable.find("span.ui-jqgrid-resize").each(function () {
                    ...
                    //2018.12.13_UPD S 位置ズレするので、表示／非表示に関わらず高さ設定を行うように修正
				...
				$htable.find("div.ui-jqgrid-sortable").each(function () {
					...
					//}
					if ($parent.is(":has(span.ui-jqgrid-resize)")) {
						$ts.css('top', ($parent.height() - $ts.outerHeight()) / 2 + 'px');
					}
                    //2018.12.13_UPD E
				});
	...
	groupingPrepare : function ( record, irow ) {
		this.each(function(){
			...
			for(i=0;i<grlen;i++) {
				...
				if( v !== undefined ) {
                    //2018.11.08_ADD S
                    //ここのロジックはcellvalueに文字列がセットされているのを前提に組まれているが、HRGridではJSONをセットしているので値を置き換える
